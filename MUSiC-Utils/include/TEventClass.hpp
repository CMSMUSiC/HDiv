...
class TEventClass : public TNamed
{
  ...
  public:
    ...
    // returns the list of ALL processes which have been filled in general (from
    // (_genInclEventClasses["Empty"])->ProcessList() )

    std::set<std::string> getProcessList()
    {
        std::set<std::string> outset = std::set<std::string>();
        for (auto &procIter : allProcHistoMap)
            outset.insert(procIter.first);
        return outset;
    }
    std::set<std::string> getGlobalProcessList()
    {
        return m_globalProcessList;
    }

    // sets the list of ALL processes contributing in general
    void setGlobalProcessList(const std::set<std::string> &processList)
    {
        m_globalProcessList = processList;
    }
    void addToGlobalProcessList(const std::string proc)
    {
        m_globalProcessList.insert(proc);
    }
    ...
    void addSystematic(std::string systName)
    {
        m_systNames.insert(systName);
    };
    // getter functions for allProcHistoMap, SystematicsProcHistoMap and PDFvariedallProcHistoMap ( containers for hists
    // in class ) Sytemati
    ...
    TH1F *getPDFHistoPointer(const std::string &process, const std::string distType, const std::string histname);
    ...
    inline std::string getClassType()
    {
        return m_eventClassType;
    };
    std::set<std::string> getSystematicNames();
    inline std::string getRunHash()
    {
        return m_runHash;
    };
    ...
    inline int getnumECItem(std::string ECItemName)
    {
        return m_countmap.at(ECItemName);
    };
    inline int getTotalNumECItems()
    {
        int count = 0;
        for (auto countpair : m_countmap)
    ...
    bool analyzedBjets()
    {
        return m_analyzedBjets;
    }
    // true if class was generated with active charge analysis
    bool analyzedCharges()
    {
        return m_analyzedCharge;
    }
    // returns absolute sum of lepton charge or -1 in case charges were off
    int getNumCharge()
    {
        return m_analyzedCharge ? m_numCharge : -1;
    }

    inline double getLumi()
    {
        return m_lumi;
    };
    ...
    inline double getScaleFactor(const std::string &process)
    {
        return m_scaleFactor[process];
    }

    inline void setScaleFactor(const std::string &process, double factor)
    {
        m_scaleFactor[process] = factor;
    }

    inline double getCrossSection(const std::string &process)
    {
        return m_crossSection[process];
    }

    inline void setCrossSection(const std::string &process, double xsec)
    {
        m_crossSection[process] = xsec;
    }

    //~ int getNumOfDistributions(const std::string process);
    unsigned int getNumOfDistributions(
    ...
    // takes the PDFvariedallProcHistoMap an creates 2 additional histograms per process & distribution (sumPT, invMass,
    // MET) indicating the upper and lower uncertainties Once the EventClasses are finally merged, we don't need the
    // individual PDF histograms any more, so they don't have to be stored in the final root file. (Also used in the
    // destructor).
    ...
    void setEventCount(const std::string &process, const unsigned int &num)
    {
        eventCount[process] = num;
    }
    void setTotalEvents(const std::string &process, const double &num)
    {
        totalEvents[process] = num;
    }
    void setTotalEventsUnweighted(const std::string &process, const double &num)
    {
        totalEventsUnweighted[process] = num;
    }
    void setTotalEventsUnweighted(const std::map<std::string, double> &nums)
    {
        totalEventsUnweighted = nums;
    }
    ...
    double getDistTypeMin(std::string distType) const
    {
        ...
        return m_distTypeMins.at(distType) >= m_distTypeMinsRequire.at(distType) ? m_distTypeMins.at(distType)
                                                                                 : m_distTypeMinsRequire.at(distType);
    }
    ...
    void setEventCount(const std::map<std::string, unsigned int> &nums)
    {
        eventCount = nums;
    }
    void setTotalEvents(const std::map<std::string, double> &nums)
    {
        totalEvents = nums;
    }
    ...
    {
        if (scanResults_pseudo_CompareScores.find(distribution) != scanResults_pseudo_CompareScores.end())
        {
    ...
    {
        if (scanResults_CompareScores.find(distribution) != scanResults_CompareScores.end())
        {
    ...
    {
        m_changelog.emplace(std::time(nullptr),
                            std::make_pair(m_eventType + m_eventClassType + ": " + change, getProcessList()));
    }
  ...
  private:
    ...
    // save the list of ALL processes which have been filled in general (from
    // (_genInclEventClasses["Empty"])->ProcessList() )
                          ...
                          std::map<std::string, std::map<std::string, std::map<std::string, TH1F *>>> &procMap)
    {
        if (procMap.find(process) == procMap.end())
            return 0;
        if (procMap[process].find(distType) == procMap[process].end())
            return 0;
        if (procMap[process][distType].find(histname) == procMap[process][distType].end())
            return 0;
    ...
    template <typename T>
    static void scaleHistoMap(std::map<std::string, T> &map, double relativeFactor);
    ...
    void applyReweighting(std::vector<TH1F *> const &hists, std::string const &process, std::string distType);
    ...
    TEventClass::PDFResult QuadSumCTEQ(std::vector<float>::const_iterator pdf_first,
                                       ...
                                       std::vector<float>::const_iterator pdf_alpha) const;
    ...
    TEventClass::PDFResult CalcMean(std::vector<float>::const_iterator pdf_first,
                                    std::vector<float>::const_iterator pdf_last) const;

    TEventClass::PDFResult CalcPairQuad(std::vector<float>::const_iterator first,
                                        std::vector<float>::const_iterator last) const;
    ...
    bool renameInMap(mapType &aMap, const keyType &oldKey, const keyType &newKey)
    {
        ...
        // swap the old and the new element, implicitly constructing the new element. the old element contains the empty
        // new object afterwards, but as it will be discarded anyway...
        ...
        {
            std::cout << "TEventClass::renameInMap: How odd, we removed no or more than 1 element from the map!"
                      << std::endl;
            return false;
        }
        return true;
    ...
    void AddToFolder(B *parent, const char *name, std::map<std::string, T> &map) const;

    template <typename B, typename T>
    void AddToFolder(B *parent, const char *name, std::vector<T> objects) const;
