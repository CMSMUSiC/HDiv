...
class ECStyleHelper(object):
    ...
    def rank_by_objects(cls, ec_name):
        ...
        object_weight_map = {
            o.name_tag: 10**i * max_object_count
            for i, o in enumerate(reversed(cls.object_types()))
        }
        prefix, counts, class_tag = cls.split_ec_name(ec_name)
        return sum(
            object_weight_map[obj.name_tag]
            * (max_object_count - counts.get(obj.name_tag, 0))
            for obj in cls.object_types()
        )

    @classmethod
    def get_ec_out_path(
        cls, basepath, ec_name, distribution, out_type="EventClass", fileformat="root"
    ):
        return os.path.join(
            basepath,
            fileformat,
    ...
    @classmethod
    def filter_class_names(
        cls, ec_names, options, object_group=None, distribution=None, **kwargs
    ):
        ...
        if hasattr(options, "object_group_tag"):
            object_group = cls.get_object_group_info_by_name_tag(
                options.object_group_tag
            )
        ...
        if distribution is not None:
            ...
            if required_objects:
                ec_names_class = set(
                    [n for n in ec_names_class if all(t in n for t in required_objects)]
                )

        # Filter for rejected objects
        ec_names_class = [
            name
            for name in ec_names_class
            if not any(n in name for n in cls.rejected_object_types())
        ]
        # Filter for required objects
        ec_names_class = [
            name
            for name in ec_names_class
            if any(n in name for n in cls.required_object_types())
        ]
    ...
    @classmethod
    def ec_class_type_loop(cls, func, conf, ec_names, **kwargs):
        """
    ...
    def add_ec_standard_options(cls, parser, **kwargs):
        parser.add_argument(
            "--filter",
            help="Apply this filter on the choice of" "EventClasses.",
            default=["*"],
            nargs="+",
        )
        parser.add_argument(
        ...
        for class_type in cls.class_types():
            arg_kwargs = {
                "help": "Use only %s classes" % class_type.name,
                "action": "store_true",
            }
            class_type_group.add_argument("--filter-" + class_type.name, **arg_kwargs)
