...
from roothelpers import (
    root_hist2arrays,
    root_get_error_hist,
    convert_ndc_axis_to_pad,
    convert_ndc_pad_to_axis,
    convert_user_to_ndc,
    get_canvas_size,
    root_setup,
    root_sum_hist,
    root_absolute_difference,
    root_sqsum_hist,
    TextAlign,
    Font,
)
...
class ECRootHelper(ECIOHelper):
    ...
    def total_event_yield(cls, event_class, distribution):
        ...
        hists = list(
            event_class.getHistoPointer(p, distribution)
            for p in list(event_class.getProcessList())
        )
    ...
    @classmethod
    def event_yield_by_process_group(
        cls, event_class, distribution, aggregation_level="medium"
    ):
        ...
        for process in list(event_class.ProcessList()):
            ...
            group = cls.get_plotting_group(process, aggregation_level)
    ...
    @classmethod
    def combined_uncertainty_by_systematic_name(
        cls,
        event_class,
        distribution,
        relative=False,
        ...
        for process in list(event_class.ProcessList()):
            ...
            for systematic in list(event_class.getSystematicNames()):
                ...
                if filter_systematics and cls._skip_systematic(
                    process, systematic, filter_systematics
                ):
                    continue
                hist_systematic = event_class.getSystHistoPointer(
                    process, distribution, systematic
                )
                ...
                if systematic.startswith("qcdWeight") and not systematic.startswith(
                    "qcdWeight%s_" % group
                ):
                ...
                if symmetrize and systematic.endswith("Up"):
                    ...
                    hist_syst_down = event_class.getSystHistoPointer(
                        process, distribution, systematic + "Down"
                    )
                    ...
                    hist_syst_diff_down = root_absolute_difference(
                        hist, hist_syst_down, sign_if_first_negative=True
                    )
                    ...
                    hist_syst_diff.Add(hist_syst_diff_up, hist_syst_diff_down, 0.5, 0.5)
        ...
        if with_stat:
            combined_systematics["MCstat"] = cls.total_combined_stat_uncertainty(
                event_class, distribution
            )
        return combined_systematics

    @classmethod
    def systematic_by_process_group(
        cls, event_class, systematic, distribution, aggregation_level="none"
    ):
    ...
    @classmethod
    def total_combined_uncertainty(
        cls, event_class, distribution, filter_systematics=[], with_stat=True
    ):
        ...
        combined_systematics = cls.combined_uncertainty_by_systematic_name(
            event_class, distribution, filter_systematics=filter_systematics
        )
    ...
    def get_plotting_group(cls, process, aggregation_level=""):
        ...
        if (
            not group
            or aggregation_level == "customDY"
            or aggregation_level == "customWJ"
        ):
            group = process
    ...
    @classmethod
    def create_line_markers(cls, plot_object, x_pos, description="", y_pos=0.92):
        ...
        xlow_canvas = ROOT.Double(0.0)
        ylow_canvas = ROOT.Double(0.0)
        xup_canvas = ROOT.Double(0.0)
        yup_canvas = ROOT.Double(0.0)
        ...
        for pad in pads:
            ...
            label = ROOT.TLatex(
                x_pos + 0.02 * (xup_canvas - xlow_canvas), y_text, description
            )
            label.SetNDC(False)
            labels.append(label)
    ...
    def generic_collision(cls, pad, box, objects):
        ...
        for object in objects:
            ...
            if "TGraphErrors" in object.ClassName():
                ...
                x_values = [
                    object.GetBinCenter(ibin + 1) for ibin in xrange(object.GetNbinsX())
                ]
                x_errors = [
                    object.GetBinWidth(ibin + 1) / 2
                    for ibin in xrange(object.GetNbinsX())
                ]
                y_values = [
                    object.GetBinContent(ibin + 1)
                    for ibin in xrange(object.GetNbinsX())
                ]
                y_errors = [0.0 for ibin in xrange(object.GetNbinsX())]
            ...
            for i in xrange(0, n_points):
                ndc_coord = convert_user_to_ndc(
                    x_values[i] + x_errors[i],
                    coordinates_canvas.xlow,
                    coordinates_canvas.xup,
                )
                converted_ndc = convert_ndc_axis_to_pad(
                    ndc_coord, left_margin, right_margin
                )
        ...
        if is_logy:
            ...
            tmp = maximum
        ymax_ndc_axis = (tmp - coordinates_canvas.ylow) / (
            coordinates_canvas.yup - coordinates_canvas.ylow
        )
        ...
        ymax_ndc_pad = convert_ndc_axis_to_pad(ymax_ndc_axis, bot_margin, top_margin)
    ...
    def calculate_new_canvas_size(cls, pad, ylow_object, maximum):
        ...
        if is_log:
            return pow(
                10,
                (math.log10(maximum) - coordinates_canvas.ylow)
                / convert_ndc_pad_to_axis(ylow_object, bot_margin, top_margin)
                + coordinates_canvas.ylow,
            )
        else:
            return (maximum - coordinates_canvas.ylow) / convert_ndc_pad_to_axis(
                ylow_object, bot_margin, top_margin
            ) + coordinates_canvas.ylow

    @classmethod
    def ec_loop(
        cls, conf, func, skip_empty=False, setup_root=True, distribution=None, **kwargs
    ):
        ...
        try:
            ...
            names, names_vetoed = cls.prepare_class_names(
                conf, mc_file=mc_file, data_file=data_file, distribution=distribution
            )
            ...
            for i, name in enumerate(sorted(names)):
                ...
                if not (skip_empty and "Empty" in name):
                    ...
                    if conf.jobs > 1:
                        args = (conf, func, name)
                        jobs.append(
                            pool.apply_async(_ec_loop_multi, args, kwargs.copy())
                        )
                    else:
                        mc_ec, data_ec = cls.read_ec_objects(mc_file, data_file, name)
                        if signal_file:
    ...
    @classmethod
    def get_distribution_width_weighted(
        cls, hist, systname, yield_histo=None, bin_min_yield=0.0001
    ):
        ...
        max_rel_deviation = 10.0
        tmp_dist = ro.TH1F(
            "sys uncert dist %s" % systname,
            "dist of %s uncerts;uncert;freq" % systname,
        ...
        if tmp_dist.Integral() > 0.0:
            tmp_dist.Scale(1.0 / tmp_dist.Integral())
    ...
    @classmethod
    def ec_root_class_type_loop(
        cls, func, conf, mc_file=None, data_file=None, **kwargs
    ):
        ...
        for class_type in ec_class_types:
            ...
            class_names, dropped = cls.prepare_class_names(
                conf, mc_file=mc_file, data_file=data_file, **all_kwargs
            )
            class_type_results[class_type.name] = func(
                conf, class_type.name, class_names, **all_kwargs
            )
            setattr(conf, class_type_option, False)
        return class_type_results

    @classmethod
    def get_class_yields(
        cls, conf, distribution="SumPt", min_yield=0.0, mc_ec=None, **kwargs
    ):
    ...
    @classmethod
    def add_ec_standard_options(
        cls, parser, add_ecout_option=True, add_parent=True, **kwargs
    ):
        ...
        parser.add_argument("--mc", help=h, type=str, default=None)
        h = "ROOT file containing a TEventClass, filled with Data events."
        parser.add_argument("--data", help=h, type=str, default=None)
        h = "ROOT file containing a TEventClass, filled with Signal events."
        parser.add_argument("--signal", help=h, type=str, default=None)
        h = "Number of parallel jobs used while processing event classes.."
        parser.add_argument("-j", "--jobs", help=h, type=int, default=1)
    ...
    def create_tex_header(
        ...
        dataset_info_text = "%.1f#kern[0.05]{fb^{-1}} (%d TeV)" % (
            lumi * 0.001,
            int(cme / 1000.0),
        )  ##sqrt{s} =
...
def ec_result_meta_dump(rdict):
    m = {"event_classes": {}, "eventclasses_distributions": {}}
    return m
