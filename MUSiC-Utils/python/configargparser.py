...
class ArgumentParser(argparse.ArgumentParser):
    ...
    def _add_config_argument(self, parser):
        ...
        group.add_argument(
            ...
            type=str,
            default=None,
            help="dump current configuration to file",
        )
        group.add_argument(
            "--dump-defaults",
            dest="dumpdefaults",
            action="store_true",
            help="also include default arguments in dump",
        )
        group.add_argument(
            "--dump-only",
            dest="dumponly",
            action="store_true",
            help="only dump the configuration and exit",
        )
    ...
    def parse_args(self, args=None):
        ...
        if tmp_namespace.dump is not None:
            self._dump_config(
                tmp_namespace.dump,
                namespace,
                include_defaults=tmp_namespace.dumpdefaults,
            )
    ...
    def get_default_namespace(self):
        ...
        for action in self._actions:
            if action.dest in self.SUPPRESSED:
                ...
                action.default == argparse.SUPPRESS or action.dest == argparse.SUPPRESS
            ):
                continue
            ...
            for name, value in parser.items(section, raw=True):
                ...
                if not action:
                    ...
                    if isinstance(action, argparse._StoreAction):
                        ...
                        action, (argparse._StoreFalseAction, argparse._StoreTrueAction)
                    ):
                    ...
                    if action:
                        if action.help:
                            ...
                            for line in wrapped_lines:
                                file.write("# " + line + "\n")
                        file.write(
                            "# " + vname + " = " + json.dumps(action.default) + "\n"
                        )
