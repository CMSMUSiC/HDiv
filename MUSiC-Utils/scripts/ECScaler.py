...
def scale_with_xsec_factor(conf, i=None, N=None, mc_ec=None, data_ec=None, **kwargs):
    ...
    if mc_ec:
        mc_ec.addChangeLogEntry(
            "Scale all cross-sections by factor %g" % conf.xsec_factor
        )
...
def parse_arguments():
    ...
    parser = configargparser.ArgumentParser(
        description="ECSaler.py - tool for scaling of TEventClasses"
    )

    ecroot.add_ec_standard_options(parser)
    parser.add_argument(
        "--lumi", help="New luminosity used to scale hists, in pb^-1", type=float
    )
    parser.add_argument(
        "--xsec-factor", help="Scale all cross sections by this factor", type=float
    )
    ...
    if not query and not conf.ecout:
        print(
            "Will not write anything to disk. Please provide an output file (--ecout) for that."
        )
        exit(1)
    return conf
