        ...
        if (startMCBinIter == m_mcBins.begin())
        {
            ...
            if (m_integralScan)
            {
                ...
                m_scanResultsCache.push_back(
                    ScanResult(mcbin, data, score, true, skipped, m_dataBins, m_totalMcEvents, m_totalMcUncert));
                break;
            }
        }
    }
    ...
    -> double
{
    ...
    for (std::size_t i = 0; i < relevant_bins.size(); i++)
    {
        if (P[i] != 0)
        {
            weight  = m_mcBins.at(relevant_bins[i]).getTotalMcUncert() ;

            res += P[i] *  log2(P[i] / Q[i]) /weight ;
                                ...
                                std::vector<int> &relevant_bins) -> double
{
    ...
    //double sum_of_elems = std::accumulate(data.begin(), data.end(), decltype(ref_model_norm)::value_type(0));
...
void ECScanner::findJSDistance()
{
    ...
    {
        if (abs(m_mcBins.at(i).getTotalMcEvents() - m_dataBins.at(i)) >= (0.0 * abs(m_mcBins.at(i).getTotalMcUncert())))
        {
            // if ((m_mcBins.at(i).getTotalMcEvents() != 0) || (m_dataBins.at(i) != 0))
            // {
            if ( abs(m_mcBins.at(i).getTotalMcUncert())  <=   0.7 * abs(m_mcBins.at(i).getTotalMcEvents() ))
            {
                if( (m_mcBins.at(i).getTotalMcEvents() >= 0.01) || ((m_dataBins.at(i)) >= 0.01))
                {
                           ...
                           bool isIntegral)
{
    ...
    if (!m_skipNeighborhoodCheck)
    {
        ...
        // Check if a leading background from the region fluctuates up / down in the
        // neighborhood this filters e.g. single spikes
...
double ECScanner::calcPvalMUSiC(const MCBin &bin, const double data) const
{
    ...
    // avoid caluclation if the difference between data and expecation is too
    // large, just set it to a small p-value because our assumptions might not be
    // valid below there.
...
void ECScanner::writeOutputFiles(const std::string outputDirectory)
{
    ...
    for (size_t i = 0; i < m_scanResults.size(); i++)
    {
        ...
        // Append line to CSV file: first 4 general fields, then info about the
        // result
