...
MCBin &MCBin::operator+=(const MCBin &add)
{
    ...
    {
        mcStatUncertPerProcessGroup[i] =
            sqrt(pow(mcStatUncertPerProcessGroup[i], 2) + pow(add.mcStatUncertPerProcessGroup[i], 2));
    }
...
bool MCBin::isEmpty() const
{
    if (!initialized)
        return true;
    if (getTotalMcEvents() != 0.)
        return false;
    if (getTotalMcUncert() != 0.)
        return false;
    return true;
}
