...
cl_library = ctypes.cdll.LoadLibrary(os.path.join(lib_path, "libConvolutionLookup.so"))
...
def c_compute_p_convolution(
    N_obs, N_SM, error_parameter, prior=DEFAULT_PRIOR, debugLevel=1
):
    return _compute_p_convolution(N_obs, N_SM, error_parameter, prior, debugLevel)


np_compute_p_convolution = np.vectorize(
    c_compute_p_convolution, excluded=["prior", "debugLevel"]
)
...
def c_lookup_p_convolution(
    n_obs, n_exp, sigma_exp, debug=False, filename=default_lookup_path
):
    return _lookup_p_convolution(n_obs, n_exp, sigma_exp, debug, filename)


np_lookup_p_convolution = np.vectorize(
    c_lookup_p_convolution, excluded=["debug", "filename"]
)
...
def c_combined_p_convolution(n_obs, n_exp, sigma_exp, prior, debug=False):
    ...
    if p < 0:
        p = c_compute_p_convolution(
            n_obs, n_exp, sigma_exp, prior, debugLevel=2 if debug else 0
        )
    return p
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="A simple tool to calculate p-values with a hybrid bayesian / frequentist approach"
    )
    parser.add_argument(
        "--batch", action="store_true", help="only print p-value and return"
    )
    parser.add_argument(
        "--lognormal",
        action="store_true",
        help="use log-normal prior instead of normal",
    )
    parser.add_argument(
        "--obs", required=True, type=int, help="observed event yield in this region"
    )
    parser.add_argument(
        "--exp", required=True, type=float, help="expected event yield in this region"
    )
    parser.add_argument(
        "--sigma-exp",
        required=True,
        type=float,
        help="total uncertainty on expected event yield",
    )
    parser.add_argument(
        "--debug",
        type=int,
        default=1,
        help="debuglevel: 0 = errors only, 1 = warnings, 2 = info, 3 = debug",
    )
    parser.add_argument(
        "--python", action="store_true", help="use python implementation"
    )
