...
def main(remote=True):
    ...
    if options.signal:
        ...
        for mc_name in mc_names:
            if not mc_name in signal_names:
                err_msg = (
                    "Class %s only in mc file but missing in signal file" % mc_name
                )
    ...
    if options.continue_run and not options.remote:
        ...
        create_and_submit(
            options, ec_names, mc_file, data_file, signal_file, hash, submitter
        )
...
def create_and_submit(
    ...
    if options.timing_json:
        ...
        class_timing_dict = {}
        default_time_per_round = 3.0

    default_width = {"SumPt": 3, "InvMass": 1, "MET": 3}
    ...
    progress = ramutils.Progress(
        maximum=len(ec_names), text="Submitting scan jobs for classes"
    )
    ...
    for iclass, name in enumerate(ec_names):
        ...
        if options.distribution not in event_class.getDistTypes():
            logger.debug(
                "Distribution %s not valid for EC %s, skipping...",
        ...
        if event_class.getTotalEvents() <= 0:
            logger.warning(
                "Negative integral (<= 0) in event class '%s', skipping..." % name
            )
            event_class.Delete()
            continue
        ...
        if data_file:
            ...
            job = job_factory.prepare_data_job(
                event_class, data_event_class, empty=(data_event_class is None)
            )
        ...
        # signalScan
            ...
            if signal_key:
                ...
                logger.debug(
                    "Creating job copies for %d signal dicing rounds.", options.Nrounds
                )
                buncher.add_class(job, runtime)
        ...
        # BG-only scan
            ...
            logger.debug(
                "Creating job copies for %d BG dicing rounds.", options.Nrounds
            )
            buncher.add_class(job, runtime)
...
class JsonBuncher(object):
    def __init__(self, options, submitter, timing_dict, hash):
        ...
        self._current_arch_runtime = 0.0
    ...
    def __del__(self):
        if self._dirty:
            logger.warning(
                "%s destroyed before all jobs have been submitted.",
    ...
    def submit(self):
        ...
        if self._dirty:
            # Construct archive
            archive_filename = "%s_%s_%d.tar.gz" % (
                self.options.remote_json_arch,
                self.hash,
                self._archive_count,
            )
            ...
            if len(json_files) > 0:
                logger.debug(
                    "Creating JSON archive '%s' containing %d JSON file(s).",
                    archive_filename,
                    len(json_files),
                )
                ...
                logger.debug(
                    "Added archive '%s' to submission queue.", archive_filename
                )
    ...
    def add_class(self, job_template, runtime):
        ...
        while start_round < self.options.Nrounds:
            remaining_rounds_in_arch = math.ceil(
                float(self.remaining_runtime) / runtime
            )
            num_rounds = int(
                min(remaining_rounds_in_arch, self.options.Nrounds - start_round)
            )
            ...
            self._split_dict[job_template.ec_name] = {
                "first_round": first_round,
                "num_rounds": num_rounds,
                "distribution": self.options.distribution,
            }
            logger.debug(
                "Adding job for '%s' containing %d rounds,"
                "expected runtime: %d seconds.",
                job_template.ec_name,
                num_rounds,
                job_runtime,
            )
            ...
            if self.remaining_runtime <= 0:
                logger.debug(
                    "Reached max archive runtime: %g sec >= %g sec, creating archive.",
                    self._current_arch_runtime,
                    self.options.runtime_per_arch,
                )
                self.submit()
            else:
                logger.debug(
                    "Maximal archive runtime (%g sec) not reached yet (currently %g sec).",
                    self.options.runtime_per_arch,
                    self._current_arch_runtime,
                )
...
class Submitter(object):
    ...
    def __init__(
        ...
        if remote:
            ...
            gridpack_remote = gp_manager.gridpack_prepare(
                scanner_base,
                [
                    "python/cewrapper.py",
                    "bin/scanClass",
            ...
            print(
                "************************ Base submitted delegation_ID scanSubmitter.py L508 ***************************"
            )  # Yannik add this
            ...
            self.remote_submitter = BaseSubmitter(
                site,
    ...
    def submit_remote(self, name, json_arch, dryrun=False, retry=10):
        print(
            "************************ Submit Remote scanSubmitter.py L529 ***************************"
        )  # Yannik add this
        """Create and submit a task using the 'cesubmit' library."""
        for tries in range(retry):
            remote_path = os.path.join(
                "store", "user", self.user, "ScannerInput/" + json_arch
            )
            try:
                self.remote_submitter.gridpack_upload(
                    os.path.abspath(json_arch), remote_path
                )
                ...
                self.logger.exception(
                    "Error uploading json arch %s. Will try again %d times in %d seconds..."
                    % (json_arch, retry - tries, wait)
                )
                time.sleep(wait)
            else:
                break
        ...
        cetask.add_gridpack(
            os.path.join("store", "user", self.user, "ScannerInput", json_arch)
        )
        ...
        cejob.arguments = ["-o", output_name, "-e", "bin/scanClass"]
    ...
    def submit_local(self, name, json_arch):
        ...
        arguments = [
            os.path.join(self.scanner_base, self.executable),
            "-o",
            out_archname,
            "-e",
            os.path.join(self.scanner_base, "bin/scanClass"),
            "-s",
            os.path.join(outer_cwd, os.path.basename(self.shifts_json)),
            "--splitting",
            "splitting.json",
        ]
...
class JobFactory(object):
    ...
    def prepare_mc_only_job(self, mc_event_class, **kwargs):
        dicing_job = DicingJob(
            mc_event_class.GetName(),
            self.distribution,
            hash=self.hash,
            config=self.config,
            **kwargs
...
class Job(object):
    ...
    def fill_mc_bins(self, event_class, filter_systematics=None):
        self.mc_bins = self._extract_mc_bins(
            event_class, self.distribution, filter_systematics
        )
    ...
    def _hists2bins(mc_hists, uncertainty_hists, unweighted_hists):
        ...
        for ibin in range(1, Nbins + 1):
            ...
            for process_name, process_hist in mc_hists.items():
                ...
                mcStatUncertPerProcessGroup[process_name] = process_hist.GetBinError(
                    ibin
                )
...
class SignalJob(DicingJob):
    ...
    def fill_signal_bins(self, event_class, filter_systematics=None):
        self.signal_bins = self._extract_mc_bins(
            event_class, self.distribution, filter_systematics
        )
...
def setup_logging(verbose_count):
    ...
    formatter = logging.Formatter(
        "%(asctime)-15s %(name)s [%(processName)-12.12s] [%(levelname)-5.5s]:  %(message)s"
    )
...
def commandline_and_config_parsing():
    ...
    parser = configargparser.ArgumentParser(
        description="Main script to run MUSiC scans"
    )
    ...
    submit_group.add_argument(
        "--scanner-base",
        default=os.path.expandvars("$SCAN_BASE"),
        type=str,
        help="Base path for the scanner",
    )
    submit_group.add_argument(
        "--executable",
        default="python/cewrapper.py",
        type=str,
        help="RoI-scan executable.",
    )
    submit_group.add_argument(
        "--out",
        default="./output",
    ...
    submit_group.add_argument(
        "--remote", action="store_true", help="Run jobs on GRID CE"
    )
    submit_group.add_argument(
        "--continue-run",
        action="store_true",
    ...
    submit_group.add_argument(
        "--timing-json",
        type=str,
        help="Name of a json file containing timing information",
    )
    submit_group.add_argument(
        "--shifts-json",
        type=str,
        help="Name of a json file containing systematic shifts",
    )
    submit_group.add_argument(
        "--site",
        type=str,
        default="T2_DE_RWTH",
        help="Run on remote CE computing grid. If not provided, the scan is ran on the local computer.",
    )
    submit_group.add_argument(
        "--gridpackname",
        type=str,
        default="scannerGridpack_HASH.tar.bz2",
        help="Name of the gridpack archive",
    )
    ...
    scan_group.add_argument(
        "-N", "--Nrounds", default=0, type=int, help="Number of dicing rounds"
    )
    scan_group.add_argument(
        "--distribution",
        choices=("SumPt", "InvMass", "MET"),
        default="SumPt",
    ...
    scan_group.add_argument(
        ...
        default=0,
        type=int,
        help="Minimum number of bins per region",
    )
    scan_group.add_argument(
        "--coverageThreshold",
        default=0.0,
        type=float,
        help="Maxmial relative MC uncertainty in regions without data.",
    )
    scan_group.add_argument(
        "--sigmaThreshold",
        default=0.0,
        type=float,
        help="Minimal deviation to calculate a p-value",
    )
    scan_group.add_argument(
        "--integralScan",
        action="store_true",
        help="Perform integral scan instead of full region construction",
    )
    scan_group.add_argument(
        "--skipLookupTable",
        action="store_true",
        help="Do not use the lookup table for p-value calculation.",
    )
    scan_group.add_argument(
        "--noLowStatsTreatment",
        default=False,
        help="Skip low statistics treatment",
        action="store_true",
    )
    scan_group.add_argument(
        "--thresholdLowStatsUncert",
        default=0.6,
        type=float,
        help="Threshold for relative statistical uncertainty in region under investigation",
    )
    scan_group.add_argument(
        "--widthLowStatsRegions",
        default=4,
        type=int,
        help="Number of bins to use as width for neighborhood regions in low statisticts treatment",
    )
    scan_group.add_argument(
        "--thresholdLowStatsDominant",
        default=0.95,
        type=float,
        help="Threshold fo quantile used to consider if a neighborhood process is dominant",
    )
    scan_group.add_argument(
        "--seed", default=None, type=int, help="Fix seed for poisson dicing"
    )
    scan_group.add_argument(
        "-s",
        "--filter_systematics",
    ...
    if args.continue_run and not args.remote:
        logging.warning(
            "Continuing previous remote submission, please make sure to use the same runtime-per-arch"
        )
