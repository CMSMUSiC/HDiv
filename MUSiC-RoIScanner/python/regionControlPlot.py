...
REASONS = {
    ...
    -91: "Rel. stat. uncert \u2265 60%",  # unicode-symbol for >=
...
class CategoryPlotter:
    def __init__(self, categories, **kwargs):
        ...
        self.categories_sorted = OrderedDict(
...
def make_control_plot(filename, args):
    ...
    try:
        ...
        if pValues.count() > 0:
            ...
            pValuesPlot = plt.pcolormesh(
                X, Y, pValues, norm=norm, cmap="gray", rasterized=True
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--minp", type=float, default=None, help="Minimal p to show on the colorbar."
    )
    parser.add_argument(
        "--maxp", type=float, default=None, help="Maximal p to show on the colorbar."
    )
