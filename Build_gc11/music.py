...
def single_test_music(total_data, signal_size, ref_model: Model, data_model: Model):
    ...
    p_data_js = read_outputs_js("outputs/music/Dummy_InvMass_output.csv")[0]
    ...
    p_toys_js = np.array(read_outputs_js("outputs/music/toys/Dummy_InvMass_output.csv"))
    # fig = plt.figure()
    # ax = plt.axes()
    # vals, bins, _ = ax.hist(p_toys_js, bins=40, label="JS Trials")
    # js_pvalue = np.sum(np.array(p_toys_js) >= p_data_js) / len(p_toys_js)
    # ax.vlines(p_data_js, 0, np.max(vals), color="red", label=f"p-value: {js_pvalue}")
    # ax.legend()
    # plt.xlabel("P-Value")
    # t = np.random.randint(0,1000)
    # fig.savefig(f"outputs/hdiv/P_dist-" +str(t) + ".png")
